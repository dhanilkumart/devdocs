        ...
        if (t.startsWith("â€¢ ") || t.startsWith("- ")) {
          return (
            <p key={i} className="flex gap-2">
              <span className="shrink-0 text-sky-600 dark:text-sky-400">â€¢</span>
              <span>
                <TechText text={t.replace(/^[â€¢-]\s*/, "")} />
              </span>
            </p>
          );
        }
...
function guessLanguage(code: string): string {
  ...
  if (code.startsWith("#") || code.includes("docker")) return "bash";
  return "javascript";
}

export function InterviewAnswerContent({ q }: { q: InterviewQuestion }) {
